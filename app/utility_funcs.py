...
def parse_deal_list_multidict(f):
    ...
    if 'add_qty' in f and 'add_prod_name' in f and 'add_name' in f and 'add_price' in f and 'add_expiry_date' in f and 'add_deal_type' in f and 'add_discount' in f:
        if len(f['add_qty']) > 0 and len(f['add_prod_name']) > 0 and len(f['add_name']) > 0 and len(f['add_price']) > 0 and len(f['add_expiry_date']) > 0 and len(f['add_deal_type']) > 0 and len(f['add_discount']) > 0:
            try:
                ...
                deal_price = 0.0
                deal_discount = 0.0
            deals.append(Deal(name=f['add_qty'], product_name = f['add_prod_name'], description=f['add_name'], price = deal_price, discount=deal_discount, category=f['add_deal_type'], expiry_date=f['add_expiry_date']))
    ...
    while next_item_name in f and next_item_prod_name in f and next_item_desc in f and next_item_price in f:
        if len(f[next_item_name]) > 0 and len(f[next_item_prod_name]) > 0 and len(f[next_item_desc]) > 0 and len(f[next_item_price]) > 0 and len(f[next_item_discount]) > 0 and len(f[next_item_category]) > 0 and len(f[next_item_expiry_date]) > 0:
            try:
                ...
                deal_price = 0
                deal_discount = 0
            deals.append(Deal(name=f[next_item_name], product_name = f[next_item_prod_name], description=f[next_item_desc], price = deal_price, discount=deal_discount, category=f[next_item_category], expiry_date=f[next_item_expiry_date]))
